...
export default function AlertsPage() {
  ...
  // Date range computation removed since data is fixed to 2025/7/7-7/14
  // const computeDateRange = useCallback(() => { ... }, [filters.period]);
  ...
  const openDetail = useCallback(async (alert: Alert) => {
    try {
      ...
      const fetchMessages = async (mode: 'fast' | 'full'): Promise<EmailThread[]> => {
        const res = await fetch(makeUrl(mode));
        if (!res.ok) return [];
