...
export async function POST(request: NextRequest) {
  try {
    console.log('🚀 スレッド表示改善版クリーンテーブル作成開始')

    // 転送サービス経由の重複メッセージを適切に処理したクリーンテーブルを作成
    const createTableQuery = `
      CREATE TABLE \`viewpers.salesguard_alerts.alerts_clean_v6\` AS
      SELECT 
        thread_id,
        message_id,
        subject,
    ...
    // 転送サービス改善の効果を確認
    const forwardingImprovementQuery = `
      SELECT 
        COUNT(*) as improved_count,
        '転送サービス改善されたメール' as description
      FROM \`viewpers.salesguard_alerts.alerts_clean_v6\`
      WHERE \`to\` NOT LIKE '%ml.cross-m.co.jp%' 
      AND customer_email NOT LIKE '%ml.cross-m.co.jp%'
      AND customer_email != 'customer@example.com'
    `

    const improvementResult = await bigquery.query({
      query: forwardingImprovementQuery,
      useLegacySql: false,
      maximumBytesBilled: '1000000000'
    })

    const improvedCount = improvementResult[0]?.[0]?.improved_count || 0

    // スレッド表示改善の効果を確認
    const threadImprovementQuery = `
      SELECT 
    ...
    const threadResult = await bigquery.query({
      query: threadImprovementQuery,
      useLegacySql: false,
      maximumBytesBilled: '1000000000'
    })

    const threadData = threadResult[0]?.[0] || {}

    return NextResponse.json({
      success: true,
      message: 'スレッド表示改善版クリーンテーブルが正常に作成されました',
      tableName: 'viewpers.salesguard_alerts.alerts_clean_v6',
      recordCount: recordCount,
      improvedForwardingCount: improvedCount,
      originalRecords: 1721,
      threadImprovement: {
        uniqueThreads: threadData.unique_threads || 0,
        totalMessages: threadData.total_messages || 0,
        avgMessagesPerThread: threadData.avg_messages_per_thread || 0
      },
      improvementFeatures: [
