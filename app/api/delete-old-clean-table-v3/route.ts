...
export async function POST() {
  try {
    ...
    // 古いクリーンテーブル（alerts_clean_v3）を削除
    ...
    return NextResponse.json({
      success: true,
      message: '古いクリーンテーブル（alerts_clean_v3）が正常に削除されました',
      deletedTable: 'viewpers.salesguard_alerts.alerts_clean_v3',
      remainingCleanTables: remainingTables,
      currentActiveTable: 'viewpers.salesguard_alerts.alerts_clean_v4'
    })

  } catch (error) {
    console.error('❌ 古いクリーンテーブル（v3）削除エラー:', error)
    return NextResponse.json(
      { 
        success: false, 
        error: '古いクリーンテーブル（v3）の削除に失敗しました',
