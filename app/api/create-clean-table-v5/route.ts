...
export async function POST(request: NextRequest) {
  try {
    ...
    // 転送サービス問題を改善したクリーンテーブルを作成
    const createTableQuery = `
      CREATE TABLE \`viewpers.salesguard_alerts.alerts_clean_v5\` AS
      SELECT 
        thread_id,
        message_id,
        subject,
    ...
    return NextResponse.json({
      success: true,
      message: '転送サービス問題改善版クリーンテーブルが正常に作成されました',
      tableName: 'viewpers.salesguard_alerts.alerts_clean_v5',
