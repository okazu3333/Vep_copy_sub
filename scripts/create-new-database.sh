...
# 新規BigQueryデータベース作成スクリプト

set -e

echo "🚀 新規BigQueryデータベース作成開始"
...
SELECT
  ...
  -- 品質スコア計算（簡略化）
  CASE 
    WHEN message_sender LIKE '=?UTF-8?B?%' OR message_sender LIKE '=?ISO-2022-JP?B?%' OR message_sender LIKE '=?Shift_JIS?B?%' THEN 1.0
    ELSE 0.0
  END as sender_decode_score,
  
  CASE 
    WHEN customer_email LIKE '=?UTF-8?B?%' OR customer_email LIKE '=?ISO-2022-JP?B?%' OR customer_email LIKE '=?Shift_JIS?B?%' THEN 1.0
    ELSE 0.0
  END as email_decode_score,
  
  CASE 
    WHEN message_subject LIKE '=?UTF-8?B?%' OR message_subject LIKE '=?ISO-2022-JP?B?%' OR message_subject LIKE '=?Shift_JIS?B?%' THEN 1.0
    ELSE 0.0
  END as subject_decode_score,
  
  CASE 
    WHEN message_body LIKE '=?UTF-8?B?%' OR message_body LIKE '=?ISO-2022-JP?B?%' OR message_body LIKE '=?Shift_JIS?B?%' THEN 1.0
    ELSE 0.0
  END as body_decode_score,
  
  -- 総合品質スコア
  (sender_decode_score + email_decode_score + subject_decode_score + body_decode_score) / 4.0 as overall_decode_score,
...
bq query --use_legacy_sql=false --location=${LOCATION} "
SELECT
  'decode_quality_summary' as metric,
...
echo ""
echo "⚡ Step 3: 検索最適化テーブル作成（5分）..."

# 3. 検索最適化テーブル
bq query --use_legacy_sql=false --location=${LOCATION} "
CREATE OR REPLACE TABLE \`${PROJECT_ID}.${TARGET_DATASET}.alerts_search_optimized\` 
PARTITION BY DATE(created_at)
CLUSTER BY priority, status, department, overall_decode_score
AS 
SELECT 
  *,
  -- 検索用ベクトル作成
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        score,
        overall_decode_score as quality,
        sender_decode_score,
        email_decode_score,
        subject_decode_score,
        body_decode_score
    ...
    return NextResponse.json({
      ...
      decodeInfo: {
        ...
        avgQuality: rows.length > 0 ? 
          rows.reduce((sum, row) => sum + (row.quality || 0), 0) / rows.length : 0
      }
    })
