#!/bin/bash
# scripts/vm-decode-during-extract.sh
# VM使用で解凍時にデコードする最適化版

set -e

echo "🚀 VM使用 解凍時デコード処理開始"
echo "📊 対象: 1ファイルでテスト"
...
INSTANCE_NAME="mbox-decoder-test"
...
# 3. 解凍時デコード処理スクリプト作成
cat > scripts/vm-decode-during-extract.js << 'EOF'
...
# 処理スクリプトをコピー
gsutil cp gs://${BUCKET_NAME}/scripts/vm-decode-during-extract.js ./vm-decode-during-extract.js
...
echo '🚀 解凍時デコード処理開始...'
time node vm-decode-during-extract.js ${TEST_ZIP_FILE}
