    ...
    const insertCustomersQuery = `
      ...
      WITH customer_stats AS (
        SELECT 
          ...
          -- 最も多くメッセージを送信した人を主要連絡先とする
          (SELECT \`from\` FROM \`${projectId}.${dataset}.unified_email_messages\` sub 
           WHERE sub.company_domain = main.company_domain AND sub.direction = 'external'
           GROUP BY \`from\` ORDER BY COUNT(*) DESC LIMIT 1) as primary_contact_email
      ...
      SELECT
        ...
        -- ドメインから会社名を推測
        CASE 
          WHEN company_domain LIKE '%.co.jp' THEN REGEXP_REPLACE(company_domain, r'\\.co\\.jp$', '')
          WHEN company_domain LIKE '%.com' THEN REGEXP_REPLACE(company_domain, r'\\.com$', '')
          WHEN company_domain LIKE '%.jp' THEN REGEXP_REPLACE(company_domain, r'\\.jp$', '')
          ELSE company_domain
        END as company_name,
        -- メッセージ量とリスク率から会社タイプを推測
        CASE 
          WHEN total_messages >= 500 THEN 'customer'
          WHEN total_messages >= 100 THEN 'partner'
          WHEN total_messages >= 20 THEN 'vendor'
          ELSE 'other'
        END as company_type,
        -- ドメインから業界を推測
        CASE 
          WHEN company_domain LIKE '%bank%' OR company_domain LIKE '%finance%' THEN 'finance'
          WHEN company_domain LIKE '%tech%' OR company_domain LIKE '%soft%' THEN 'technology'
          WHEN company_domain LIKE '%media%' OR company_domain LIKE '%news%' THEN 'media'
          WHEN company_domain LIKE '%recruit%' OR company_domain LIKE '%hr%' THEN 'hr'
          WHEN company_domain LIKE '%marketing%' OR company_domain LIKE '%ad%' THEN 'marketing'
          ELSE 'other'
        END as industry,
        -- メッセージ量から会社規模を推測
        CASE 
          WHEN total_messages >= 1000 THEN 'enterprise'
          WHEN total_messages >= 300 THEN 'large'
          WHEN total_messages >= 50 THEN 'medium'
          ELSE 'small'
        END as company_size,
        -- リスク率からリスクレベルを設定
        CASE 
          WHEN risk_rate >= 50 THEN 'critical'
          WHEN risk_rate >= 25 THEN 'high'
          WHEN risk_rate >= 10 THEN 'medium'
          ELSE 'low'
        END as risk_level,
        total_messages,
        unique_contacts,
        ...
        -- 最近の活動から状態を推測
        CASE 
          WHEN DATE_DIFF(CURRENT_DATE(), DATE(last_contact), DAY) <= 30 THEN 'active'
          WHEN DATE_DIFF(CURRENT_DATE(), DATE(last_contact), DAY) <= 90 THEN 'inactive'
          ELSE 'prospect'
        END as status
      FROM customer_stats
