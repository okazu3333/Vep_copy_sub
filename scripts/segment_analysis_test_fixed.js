...
(async () => {
  try {
    ...
    const [contentResult] = await bq.query({ query: contentQuery, useLegacySql: false });
    console.log('実際のメッセージ内容サンプル:');
    contentResult.forEach((row, i) => {
      console.log(`  ${i+1}. [${row.primary_risk_type}] スコア${row.score} (${row.direction})`);
      console.log(`     件名: ${row.subject?.substring(0, 80)}...`);
      console.log(`     本文: ${row.body_preview?.substring(0, 80)}...`);
      console.log('');
    });
