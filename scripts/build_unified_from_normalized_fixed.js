...
(async () => {
  try {
    ...
    console.log('=== email_messages_normalized ベース統合テーブル構築 (修正版) ===');
    console.log('REPEATED フィールド対応 + CC/BCC重複処理');
    console.log('');
    ...
    const createTableDDL = `
      ...
      OPTIONS (
        description = "Unified email messages based on email_messages_normalized with proper array handling"
      )
    `;
    ...
    // Step 2: Insert data with proper array handling
    console.log('');
    console.log('📊 Step 2: normalized ベースでデータ挿入 (配列対応)');
    const insertQuery = `
    ...
    console.log('実行中: データ挿入 (配列→文字列変換)...');
    ...
    console.log('');
    console.log('✅ 完了した機能:');
    console.log('  • CC/BCC重複を適切に処理 (message_id単位で重複除去)');
    console.log('  • REPEATED フィールドの正しい処理');
    ...
    console.log('  • 左右分割表示 (internal/external)');
