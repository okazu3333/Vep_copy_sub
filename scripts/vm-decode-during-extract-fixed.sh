#!/bin/bash
# scripts/vm-decode-during-extract-fixed.sh
# VM使用で解凍時にデコードする最適化版（修正版）

set -e

echo "🚀 VM使用 解凍時デコード処理開始（修正版）"
echo "📊 対象: 1ファイルでテスト"
...
INSTANCE_NAME="mbox-decoder-test-fixed"
MACHINE_TYPE="c2-standard-8"  # 8 vCPU, 32GB RAM
BOOT_DISK_SIZE="200GB"
BUCKET_NAME="salesguarddata"
TEST_ZIP_FILE="cm-test-20250707-1w--a_fukuda@withwork.co.jp-gvFg5n.mbox.zip"

echo ""
echo "📋 Step 1: テスト用VM作成（5分）..."
...
# 3. 解凍時デコード処理スクリプト作成
cat > scripts/vm-decode-during-extract-fixed.js << 'EOF'
...
# 処理スクリプトをコピー
gsutil cp gs://${BUCKET_NAME}/scripts/vm-decode-during-extract-fixed.js ./vm-decode-during-extract-fixed.js

echo '✅ 依存関係インストール完了'
"

echo ""
echo "⚡ Step 5: 解凍時デコード処理実行（30分）..."
...
echo '🚀 解凍時デコード処理開始...'
time node vm-decode-during-extract-fixed.js ${TEST_ZIP_FILE}
...
FROM \`${PROJECT_ID}.salesguard_alerts_new.alerts_decode_during_extract\`
"
...
echo ""
echo "🎉 VM使用 解凍時デコード処理完了！"
echo ""
echo "📊 処理結果:"
echo "   - 対象ファイル: ${TEST_ZIP_FILE}"
echo "   - 処理時間: 約30分"
echo "   - VMコスト: c2-standard-8 × 0.5時間 = $0.14"
echo "   - BigQueryコスト: 約$0.50"
echo "   - 総コスト: 約$0.64"
echo ""
echo "🔗 アクセス方法:"
echo "   - テーブル: alerts_decode_during_extract"
