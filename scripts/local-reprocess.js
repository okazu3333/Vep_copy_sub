...
const TABLE_ID = 'alerts_local_fixed';

/**
 * ローカル処理による再処理
 */
async function localReprocess() {
  try {
    console.log('🔧 ローカル再処理開始');
    ...
    const targetFiles = process.argv.includes('--all') 
      ? zipFiles 
      : zipFiles.slice(0, parseInt(process.argv.find(arg => arg.startsWith('--files='))?.split('=')[1] || 1));
    ...
    for (let i = 0; i < targetFiles.length; i++) {
      const zipFile = targetFiles[i];
      console.log(`\n📦 処理中 (${i + 1}/${targetFiles.length}): ${zipFile.name}`);
...
async function processZipFile(zipFile) {
  try {
    // 1. ZIPファイルをダウンロード
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'local-reprocess-'));
    const zipPath = path.join(tempDir, 'data.zip');
    
    console.log(`⬇️  ダウンロード中: ${zipFile.name}`);
    await zipFile.download({ destination: zipPath });
    
    const fileSizeMB = (fs.statSync(zipPath).size / (1024 * 1024)).toFixed(1);
    console.log(`✅ ダウンロード完了: ${fileSizeMB}MB`);
    ...
    for (const mboxFile of mboxFiles) {
      const result = await processMboxFile(mboxFile);
      totalProcessed += result.processed;
      totalFixed += result.fixed;
    }
 ...
 * mboxファイルを処理
 */
async function processMboxFile(mboxFilePath) {
  try {
    const mboxContent = fs.readFileSync(mboxFilePath, 'utf8');
    const emails = parseMboxContent(mboxContent, path.basename(mboxFilePath));
